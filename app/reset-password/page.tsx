...
import React, { useState, useEffect, Suspense } from "react"; // Tambah Suspense
import { useSearchParams, useRouter } from "next/navigation";
import { Key, AlertTriangle, Loader2, ArrowLeft, CheckCircle2 } from "lucide-react";
import Link from "next/link";

// 1. Pindahkan seluruh logic lama ke komponen internal ini
      ...
      {!status || status.type !== "success" ? (
        <form onSubmit={handleSubmit} style={{ background: "#FFFFFF", padding: "24px", borderRadius: "16px", border: "1.5px solid #E8E4DC", boxShadow: "0 4px 20px rgba(0,0,0,0.03)" }}>
          <div style={{ marginBottom: "16px" }}>
            ...
            <input type="password" required placeholder="••••••••" style={inputStyle} value={password} onChange={(e) => setPassword(e.target.value)} disabled={!token || isSubmitting} />
          </div>
          <div style={{ marginBottom: "24px" }}>
            ...
            <input type="password" required placeholder="••••••••" style={inputStyle} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={!token || isSubmitting} />
          </div>
          <button type="submit" disabled={!token || isSubmitting} style={{ width: "100%", padding: "12px", background: "#F0A500", border: "none", borderRadius: "10px", fontSize: "14px", fontWeight: 700, color: "#1A1A1A", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", transition: "0.2s", boxShadow: "0 4px 12px rgba(240, 165, 0, 0.2)" }}>
...
// 2. Eksport utama yang membungkus konten dengan Suspense
    ...
    <div style={{ 
      ...
      <Suspense fallback={
        <div style={{ textAlign: "center", color: "#1A1A1A" }}>
          ...
          <p style={{ fontSize: "14px", fontWeight: 500 }}>Memuat halaman...</p>
        </div>
      }>
