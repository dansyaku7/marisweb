  ...
  return (
    <>
      <style>{`
        ...
        .eq-search-icon  { position:absolute; left:11px; top:50%; transform:translateY(-50%); color:#1A1A1A; pointer-events:none; } /* UBAH KE ITEM */
        ...
        .eq-search-input::placeholder { color:#AAAAAA; }
        ...
        .eq-tool-btn             { display:flex; align-items:center; gap:7px; padding:8px 14px; background:#FFFFFF; border:1.5px solid #E5E2D8; border-radius:8px; font-family:inherit; font-size:12px; font-weight:500; color:#1A1A1A; cursor:pointer; white-space:nowrap; transition:0.2s; } /* UBAH KE ITEM */
        ...
        .eq-permit, .eq-insp-date, .eq-expiry { font-size:12px; color:#1A1A1A; font-family:monospace; } /* SEMUA JADI ITEM */
        ...
        .action-btn           { padding:6px; border-radius:7px; cursor:pointer; background:#F5F3EE; border:1.5px solid #E5E2D8; color:#1A1A1A; transition:0.2s; } /* IKON ITEM */
        ...
        .pag-info { font-size:10px; color:#1A1A1A; font-family:monospace; white-space:nowrap; } /* INFO PAG ITEM */
        ...
        .modal-subtitle { font-size:12px; color:#1A1A1A; margin-bottom:20px; } /* SUBTITLE ITEM */
        .modal-label    { display:block; font-size:11px; font-weight:600; color:#1A1A1A; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:6px; } /* LABEL ITEM */
              ...
              <div className="eq-card" style={{ display: "flex", flexDirection: "column" }}>
                          ...
                          return (
                            <tr key={eq.id} style={{ background: isSelected ? "#FDFCF8" : "transparent" }}>
                              ...
                              <td className="sticky-col" style={{ 
                                position: "sticky", left: 40, background: stickyBg, zIndex: 10, boxShadow: "2px 0 5px rgba(0,0,0,0.02)",
                              ...
                              <td><span className="eq-insp-date">{new Date(eq.inspectionDate).toLocaleDateString("id-ID")}</span></td>
                              <td><span className="eq-expiry">{new Date(eq.expiryDate).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" })}</span></td>
                ...
                {filteredEquipments.length > 0 && (
                  <div className="pag-bar">
                    ...
                    <div className="pag-btns">
                      <button className="pag-btn" disabled={eqPage === 1} onClick={() => setEqPage((p) => p - 1)}><ChevronLeft size={12} /> Prev</button>
                      <button className="pag-btn" disabled={eqPage === totalEqPages} onClick={() => setEqPage((p) => p + 1)}>Next <ChevronRight size={12} /></button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <DocumentDrawer equipment={activeEq} isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} userRole={userRole} onDocumentSaved={() => fetchEquipments(selectedCompanyId)} />
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget && !isSubmittingEdit) setIsEditModalOpen(false); }}>
          <div className="modal-content">
            ...
            <form onSubmit={handleEditSubmit} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {renderFormFields(editingEq, (key, val) => setEditingEq((prev) => prev ? { ...prev, [key]: val } : prev))}
        ...
        <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget && !isSubmittingAdd) setIsAddModalOpen(false); }}>
          <div className="modal-content">
            ...
            <form onSubmit={handleAddSubmit} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {renderFormFields(newEq, (key, val) => setNewEq((prev) => ({ ...prev, [key]: val })))}
              <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
