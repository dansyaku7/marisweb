...
export default function EquipmentsPage() {
  ...
  const handleClientRequest = async (type: 'inspeksi' | 'dokumen') => {
    ...
    const ok = await confirm({
      variant: "info", title: "Kirim Permohonan",
    ...
    if (!ok) return;
    setIsSubmittingRequest(true); setStatusMsg(null);
    try {
      const res = await fetch("/api/equipments/client-request", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ equipmentIds: selectedEqIds, requestType: type }),
      });
  ...
  const handleAdminNotify = async (type: 'expired' | 'ready') => {
    ...
    const config = type === 'expired'
      ? { title: "Alert Expired", label: "Peringatan Expired", variant: "danger" as const }
      : { title: "Notif Dokumen", label: "Notif Dokumen Ready", variant: "info" as const };
    const ok = await confirm({
      variant: config.variant, title: config.title,
    ...
    try {
      const res = await fetch(`/api/companies/${selectedCompanyId}/notify-bulk`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ selectedIds: selectedEqIds, type }),
      });
      const data = await res.json();
  ...
  return (
    <>
      <style>{`
        ...
        /* ↓ Tambah paddingBottom 80px supaya konten bawah ga tertutup */
        .eq-inner { padding:28px 20px 80px; max-width:1280px; margin:0 auto; animation: eq-fadeup 0.4s ease both; }
        @media (min-width:768px)  { .eq-inner { padding:40px 32px 80px; } }
        @media (min-width:1024px) { .eq-inner { padding:48px 48px 80px; } }
        ...
        .eq-page-desc  { font-size:13px; color:#1A1A1A; margin:0; }
        ...
        .eq-card-title  { font-size:13px; font-weight:600; color:#1A1A1A; }

        .eq-company-list { padding:8px; display:flex; flex-direction:column; gap:2px; }
        .eq-company-btn       { width:100%; text-align:left; padding:10px 14px; border-radius:9px; background:transparent; border:1.5px solid transparent; font-family:inherit; font-size:13px; font-weight:500; color:#1A1A1A; cursor:pointer; display:flex; align-items:center; justify-content:space-between; gap:8px; transition:0.15s; }
        ...
        .eq-search-icon  { position:absolute; left:11px; top:50%; transform:translateY(-50%); color:#1A1A1A; pointer-events:none; }
        ...
        .eq-tool-btn             { display:flex; align-items:center; gap:7px; padding:8px 14px; background:#FFFFFF; border:1.5px solid #E5E2D8; border-radius:8px; font-family:inherit; font-size:12px; font-weight:500; color:#1A1A1A; cursor:pointer; white-space:nowrap; transition:0.2s; }
        ...
        .eq-table thead th { padding:12px 16px; font-size:10px; font-weight:600; text-transform:uppercase; letter-spacing:0.1em; color:#1A1A1A; border-bottom:1px solid #F0EDE4; white-space:nowrap; }
        ...
        .eq-permit, .eq-insp-date, .eq-expiry { font-size:12px; color:#1A1A1A; font-family:monospace; }
        ...
        .eq-empty-icon  { width:56px; height:56px; background:#F5F3EE; border:1.5px solid #E5E2D8; border-radius:14px; display:flex; align-items:center; justify-content:center; margin:0 auto 16px; color:#1A1A1A; }
        .eq-empty-title { font-size:14px; font-weight:600; color:#1A1A1A; margin:0 0 6px; }
        .eq-empty-desc  { font-size:12px; color:#1A1A1A; margin:0; }
        .eq-spinner     { animation: eq-spin 1s linear infinite; }

        .action-btn           { padding:6px; border-radius:7px; cursor:pointer; background:#F5F3EE; border:1.5px solid #E5E2D8; color:#1A1A1A; transition:0.2s; }
        ...
        .doc-btn       { padding:5px 11px; font-size:11px; font-weight:500; border-radius:7px; cursor:pointer; display:inline-flex; align-items:center; gap:6px; transition:0.2s; border:1.5px solid #E5E2D8; font-family:inherit; background:#F5F3EE; color:#1A1A1A; }
        ...
        .pag-info { font-size:10px; color:#1A1A1A; font-family:monospace; white-space:nowrap; }
        .pag-btns { display:flex; gap:6px; }
        .pag-btn  { padding:5px 10px; font-size:11px; font-weight:500; border-radius:7px; background:#FFFFFF; border:1.5px solid #E5E2D8; color:#1A1A1A; cursor:pointer; display:flex; align-items:center; gap:3px; transition:0.15s; font-family:monospace; }
        ...
        .modal-subtitle { font-size:12px; color:#1A1A1A; margin-bottom:20px; }
        .modal-label    { display:block; font-size:11px; font-weight:600; color:#1A1A1A; text-transform:uppercase; letter-spacing:0.08em; margin-bottom:6px; }
        .modal-btn-cancel { flex:1; padding:11px; background:#F5F3EE; border:1.5px solid #E5E2D8; border-radius:10px; font-family:inherit; font-size:13px; font-weight:500; color:#1A1A1A; cursor:pointer; transition:0.15s; }
        ...
        /* ─── AKSI COLUMN: sticky hanya di desktop ≥768px ─── */
        .aksi-th, .aksi-td {
          text-align: center;
            ...
            <div className="eq-right">
              <div className="eq-card" style={{ display: "flex", flexDirection: "column" }}>
                <div className="eq-toolbar">
                  ...
                  {userRole !== "SUPERADMIN" && (
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                      <button className="eq-tool-btn amber" onClick={() => handleClientRequest('inspeksi')} disabled={selectedEqIds.length === 0 || isSubmittingRequest}>
                        {isSubmittingRequest ? <Loader2 size={13} className="eq-spinner" /> : <Send size={13} />}
                        Request Inspeksi
                      </button>
                      <button className="eq-tool-btn amber" onClick={() => handleClientRequest('dokumen')} disabled={selectedEqIds.length === 0 || isSubmittingRequest}>
                    ...
                    <table className="eq-table">
                      <thead>
                        <tr>
                          ...
                          <th>Status</th>
                          <th>Keterangan</th>
                          {/* ↓ Pakai class aksi-th — sticky hanya di desktop */}
                          <th className="aksi-th" style={{ background: "#FAFAF7" }}>Aksi</th>
                        </tr>
                      </thead>
                      <tbody>
                          ...
                          return (
                            <tr key={eq.id} style={{ background: isSelected ? "#FDFCF8" : "transparent" }}>
                              ...
                              <td className="sticky-col" style={{
                                position: "sticky", left: 40, background: stickyBg, zIndex: 10,
                                boxShadow: "2px 0 5px rgba(0,0,0,0.02)",
                                whiteSpace: "normal", maxWidth: 220, wordBreak: "break-word", lineHeight: 1.4,
                              }}>
                                <p className="eq-eq-name" onClick={() => { setActiveEq(eq); setDrawerOpen(true); }}>
                                  {eq.name}
                                  {eq.name.toLowerCase() === 'lift' || eq.name.toLowerCase() === 'crane'
                                    ? ` - ${eq.area || eq.location || eq.serialNumber}`
                                    : ''}
                                </p>
                              </td>
                              ...
                              <td>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', alignItems: 'flex-start' }}>
                                  ...
                                  {eq.isProsesDinas && (
                                    <span className="eq-badge process">
                              ...
                              {/* ↓ Pakai class aksi-td — sticky hanya di desktop */}
                              <td className="aksi-td" style={{ background: stickyBg }}>
