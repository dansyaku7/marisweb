...
export default function CompaniesPage() {
  ...
  const openDelete = (company: Company) => {
    setDeleteTarget(company); setDeleteError(null); setIsDeleteOpen(true);
  };
  ...
  const filteredCompanies = companies.filter((c) =>
    ...
    c.emailPic.toLowerCase().includes(searchText.toLowerCase()) ||
    (c.namePic || "").toLowerCase().includes(searchText.toLowerCase())
  );
  ...
  return (
    <>
      <style>{`
        ...
        .cp-inner { padding:28px 20px; max-width:1280px; margin:0 auto; animation:cp-fadeup 0.4s ease both; }
        @media (min-width:768px)  { .cp-inner { padding:40px 32px; } }
        @media (min-width:1024px) { .cp-inner { padding:48px 48px; } }

        .cp-page-header { display:flex; flex-wrap:wrap; align-items:flex-end; justify-content:space-between; gap:20px; margin-bottom:32px; }
        ...
        /* Modal shared */
        .cp-modal-overlay { position:fixed; inset:0; z-index:100; display:flex; align-items:center; justify-content:center; padding:20px; overflow-y:auto; }
        .cp-modal-bg { position:absolute; inset:0; background:rgba(0,0,0,0.25); backdrop-filter:blur(6px); }
        .cp-modal    { position:relative; background:#FFFFFF; border:1.5px solid #EAE7DF; border-radius:20px; width:100%; max-width:480px; overflow:hidden; box-shadow:0 24px 60px rgba(0,0,0,0.12); animation:cp-modal-in 0.2s ease both; margin:auto; }
        .cp-modal-sm { position:relative; background:#FFFFFF; border:1.5px solid #EAE7DF; border-radius:20px; width:100%; max-width:360px; overflow:hidden; box-shadow:0 24px 60px rgba(0,0,0,0.12); animation:cp-modal-in 0.2s ease both; margin:auto; }
        ...
        .cp-btn-delete-confirm { flex:1; padding:11px; background:rgba(220,60,60,0.07); border:1.5px solid rgba(220,60,60,0.18); border-radius:10px; font-family:inherit; font-size:13px; font-weight:600; color:#DC3C3C; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:6px; transition:0.15s; }
              ...
              <table className="cp-table">
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr><td colSpan={7}>
                      <div className="cp-empty-cell">
                        ...
                        <p className="cp-empty-title">
                          {searchText ? "Tidak ada hasil pencarian" : "Belum ada klien"}
                        </p>
                        <p className="cp-empty-desc">
                          {searchText
                            ? `Tidak ditemukan klien yang cocok dengan "${searchText}".`
                            : "Klik tombol Tambah Klien untuk memulai."}
                        </p>
                      </div>
                    </td></tr>
                    ...
                    <tr key={company.id}>
                      <td>
                        <p className="cp-company-name">{company.name}</p>
                      </td>
                      <td>
                        {company.namePic ? (
          ...
          <div className="cp-modal">
            ...
            <form onSubmit={handleSubmit}>
              <div className="cp-modal-body">
                {formError && (
                  <div className="cp-form-error"><AlertCircle size={14} style={{ flexShrink: 0 }} />{formError}</div>
                )}

                {/* Perusahaan */}
                <p className="cp-form-section">Informasi Perusahaan</p>
                <div className="cp-field">
                  ...
                  <div className="cp-field-wrap">
                    ...
                    <input type="text" required className="cp-field-input"
                      placeholder="Contoh: PT. Delta Anugerah"
                      value={formData.name}
                ...
                {/* PIC */}
                <p className="cp-form-section">Data PIC (Person in Charge)</p>
                <div className="cp-field-row" style={{ marginBottom: 16 }}>
                  <div className="cp-field">
                    <label className="cp-field-label">Nama PIC</label>
                    <div className="cp-field-wrap">
                      <User size={14} className="cp-field-icon" />
                      <input type="text" className="cp-field-input"
                        placeholder="Nama lengkap PIC"
                        value={formData.namePic}
                        onChange={(e) => setFormData({ ...formData, namePic: e.target.value })} />
                    </div>
                  </div>
                  <div className="cp-field">
                    <label className="cp-field-label">No HP PIC</label>
                    <div className="cp-field-wrap">
                      <Phone size={14} className="cp-field-icon" />
                      <input type="tel" className="cp-field-input"
                        placeholder="08xx-xxxx-xxxx"
                        value={formData.phonePic}
                        onChange={(e) => setFormData({ ...formData, phonePic: e.target.value })} />
                    </div>
                  </div>
                </div>

                {/* Akun */}
                <p className="cp-form-section">Akun Akses</p>
                <div className="cp-field">
                  <label className="cp-field-label">Email Akses (PIC)</label>
                  <div className="cp-field-wrap">
                    <Mail size={14} className="cp-field-icon" />
                    <input type="email" required className="cp-field-input"
                ...
                <div className="cp-field" style={{ marginBottom: 0 }}>
                  ...
                  <div className="cp-field-wrap">
                    ...
                    <input type="password" required minLength={6} className="cp-field-input"
        ...
        <div className="cp-modal-overlay">
          ...
          <div className="cp-modal">
            ...
            <form onSubmit={handleEdit}>
              <div className="cp-modal-body">
                {editError && (
                  <div className="cp-form-error"><AlertCircle size={14} style={{ flexShrink: 0 }} />{editError}</div>
                )}
                ...
                <p className="cp-form-section">Data PIC</p>
                <div className="cp-field-row" style={{ marginBottom: 16 }}>
                  <div className="cp-field">
                    <label className="cp-field-label">Nama PIC</label>
                    <div className="cp-field-wrap">
                      ...
                      <input type="text" className="cp-field-input"
                        placeholder="Nama lengkap PIC"
                        value={editData.namePic}
                        onChange={(e) => setEditData({ ...editData, namePic: e.target.value })} />
                    </div>
                  </div>
                  <div className="cp-field">
                    <label className="cp-field-label">No HP PIC</label>
                    <div className="cp-field-wrap">
                      ...
                      <input type="tel" className="cp-field-input"
                        placeholder="08xx-xxxx-xxxx"
                        value={editData.phonePic}
                        onChange={(e) => setEditData({ ...editData, phonePic: e.target.value })} />
                    </div>
                  </div>
                </div>
                ...
                <div className="cp-field">
                  ...
                  <div className="cp-field-wrap">
                    ...
                    <input type="email" required className="cp-field-input"
                      placeholder="pic@perusahaan.com"
                      value={editData.emailPic}
                      onChange={(e) => setEditData({ ...editData, emailPic: e.target.value })} />
                  </div>
                </div>
